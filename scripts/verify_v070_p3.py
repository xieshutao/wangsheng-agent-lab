...
GREEN_TESTS = {f"T{index:02d}" for index in range(1, 17)}
RED_TESTS = {f"T{index:02d}" for index in range(17, 21)}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="v070-p3-") as temp_dir:
        ...
        internal_total = 0
        internal_passed = 0
        unexpected_internal_failures: list[str] = []
            ...
            for testcase in xml_root.iter("testcase"):
                ...
                if test_id is None:
                    ...
                    if node is None:
                        ...
                        unexpected_internal_failures.append(name)
                    continue
                seen.add(test_id)
                if node is None:
                    passed.add(test_id)
    ...
    unexpected_contract_failures = sorted(set(failed) - marker_failures)
    summary = {
        ...
        "unexpected_internal_failures": sorted(unexpected_internal_failures),
    ...
    summary["status"] = (
        ...
        and not unexpected_contract_failures
        and internal_total == internal_passed
        and not unexpected_internal_failures
