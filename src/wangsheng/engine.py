...
class EpisodeEngine:
    ...
    def tick(self) -> Observation:
        task = self._require_task()
        if task.is_terminal:
            return Observation(False, "task_terminal", "No tick after terminal task.", Action("wait"))
        context = self._build_context(task)
        action = self.policy.next_action(context)
        rejection = self.gateway.validate(action=action, task=task, world=self.world)
        observation = rejection if rejection is not None else self.executor.execute(action=action, world=self.world)
        task.step_count += 1
    ...
    def run_until_terminal(self) -> ActiveTask:
        task = self._require_task()
        while not task.is_terminal: self.tick()
        return task
    def _build_context(self, task: ActiveTask) -> PolicyContext:
        return PolicyContext(task.spec.command, task.spec.task_id, task.step_count,
                             tuple(sorted(task.spec.allowed_actions)),
                             tuple(sorted(task.spec.forbidden_actions)),
                             self.world.snapshot(),
                             tuple(o.to_dict() for o in task.observations))
    def _require_task(self) -> ActiveTask:
        if self.active_task is None: raise RuntimeError("No active task. Call submit_command first.")
        return self.active_task
